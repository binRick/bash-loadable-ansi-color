...
#define START_VLA(type, n, varname)                  \
    ...
    if (sizeof(vla) == 0) {                          \
        ...
        if (varname == NULL) {                       \
            printf("malloc %zu failed", n * sizeof(type)); \
...
#define START_VLA2(type, n, varname)                 \
    ...
    do {                                             \
        if (sizeof(vla) != 0) {                      \
            ...
            if (varname == NULL) {                   \
                printf("calloc %zu failed", n * sizeof(type)); \
                return (EXECUTION_FAILURE);          \
            }                                        \
        }                                            \
    } while (0)
...
int parse_errno(const char *arg, size_t i, const char *fname)
{
    ...
    if (arg[0] != 'E') {
        printf("%s: %s: the %zu arg isn't errno", fname, self_name, i);
        return -1;
    }
    ++arg;
